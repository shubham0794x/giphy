...
class App extends Component {
  ...
  doSearch = newSearch => {
    ...
    if (!searchParam) {
      this.setState({ errorMsg: 'You must enter some search parameter   ' });
      this.hideLoading();
      return;
    }
  ...
  renderLoadMoreButton = () => {
    if (this.state.data.length) {
      return (
        <div>
          <SearchButton onClick={this.doSearch.bind(this, false)} text="Load More" />
        </div>
      );
    }
  ...
  render() {
    return (
      ...
      >
        <div className="App">
          <Header headerText="GIFt’ed : Giphy Search Engine" />
          <SearchKeyword buttonText="Search"
